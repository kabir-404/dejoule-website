      ...
      <div className="max-w-[1580px] mx-auto px-6 lg:px-12">
        <div className="flex items-center justify-between h-[72px] lg:h-[88px]">
          ...
          <Link href="/" className="flex flex-col items-start shrink-0">
            ...
            />
            <span
              className="text-[7px] tracking-[0.08em] font-light mt-0.5"
              style={{ color: "#ca3604", fontFamily: "var(--font-geist-sans), sans-serif" }}
            >
          ...
          <button onClick={() => setMobileOpen(!mobileOpen)}
            ...
            <span className="flex flex-col gap-1.5 w-5">
              ...
              <span className={`h-0.5 bg-current rounded-full transition-all duration-300 ${mobileOpen ? "opacity-0 scale-x-0" : ""}`} />
              <span className={`h-0.5 bg-current rounded-full transition-all duration-300 ${mobileOpen ? "-rotate-45 -translate-y-2" : ""}`} />
            </span>
          </button>
        </div>
          ...
          <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: "auto" }}
            ...
            <div className="px-4 py-5 space-y-1 bg-white">
              {navLinks.map((link) =>
                link.dropdown ? (
                  <div key={link.label}>
                    <p className="px-3 py-1.5 text-xs font-semibold text-gray-400 uppercase tracking-widest">{link.label}</p>
                    {link.dropdown.map((item) => (
